...
#
# below line is for mappi = rpi5 running raspberrypi os.
df -h | grep -vE '^Filesystem|udev|tmpfs|tmpfs|tmpfs|/dev/nvme0n1p1|tmpfs' | awk '{ print $4 " " $1 }' | while read -r output;
do
  echo "$output"
