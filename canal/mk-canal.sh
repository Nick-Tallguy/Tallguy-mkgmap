...
java -Xmx14g -jar ${GHUB}/mkgmap-progs/splitter-r654/splitter.jar --output=pbf --output-dir=${GMAKE}/splitter --max-nodes=1400000 --mapid=10010001 --geonames-file=${GHUB}/mkgmap-progs/cities15000.zip   --polygon-file=${POLY} ${PBF}
PROCESS_RETURN
#
### MKGMAP 
echo "Starting mkgmap" $(date -u)
rm -r ${GMAKE}/work/*
java -Xms1024m -Xmx14g  -jar ${GHUB}/mkgmap-progs/mkgmap-r4923/mkgmap.jar -c ${NC_STYLES}/nick.args --family-name=${FAMILYNME} -c ${GMAKE}/splitter/template.args --description="Tallguy - canal" ${NC_STYLES}/5410.txt --gmapsupp --gmapi --nsis   
PROCESS_RETURN
##
...
transmission-create ${NME}-winexe-${DATE}.torrent -c ${DESC} -t udp://tracker.opentrackr.org:1337/announce -t udp://open.demonii.com:1337/announce -t https://tracker1.520.jp:443/announce ${ZIPPED}/${NME}-winexe-${DATE}.7z
PROCESS_RETURN
# 
